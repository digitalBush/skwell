...
const { valueToType, isObj } = require( "src/typeMapper" );
...
describe( "typeMapper", () => {
	...
	it("isObj", ()=>{
		...
		isObj({test: "lol"}).should.be.true();

		isObj([]).should.be.false();
		isObj(undefined).should.be.false();
		isObj(null).should.be.false();
		isObj("lol").should.be.false();
		isObj(types.nvarchar(100)).should.be.false();
		isObj(new Set()).should.be.false();
		isObj(123).should.be.false();
		isObj(new Date()).should.be.false();
	})
} );
